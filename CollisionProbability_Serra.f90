...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    function Serra (x, y, sigmax, sigmay, R, N) result (Pc)    
        ...
        integer, intent(in) :: N
        real(kind=8) :: x, y, sigmax, sigmay
        real(kind=8) :: Pc, p, phi, wx, wy, alpha0 
        integer ::  i
        real(kind=8), dimension(0:max(N-1, 0)) :: c
...
! if this is not true, we swap the axes using p as dummy variable
        ...
        ! Auxiliary Variables
        p = 1D0 / ( 2D0 * sigmay**2 )
        phi = 1D0 - ( sigmay / sigmax )**2
        wx = x**2 / ( 4D0 * sigmax**4 )
        wy = y**2 / ( 4D0 * sigmay**4 )
        alpha0 = exp( -( (x/sigmax)**2 + (y/sigmay)**2 )/ 2D0 ) / ( 2.0D0 * sigmax * sigmay )

        !Calculating the terms in the series
        c(0) = R**2
        if (N > 1) then
            c(1) = R**4 / 2D0 * ( p*(phi/2D0 + 1D0) + wx + wy )
            if (N > 2) then 
                c(2) = R**6 / 12D0 * ( ( p*(phi/2D0+1D0) + wx + wy )**2 +  p**2*(phi**2/2D0 + 1D0) + 2D0*p*phi*wx )
                if (N > 3) then
                    c(3) = R**8 / 144D0 *  ( ( p*(phi/2+1) + wx + wy)**3 + 3*(p*(phi/2+1) + wx + wy) * ( p**2*(phi**2/2D0+1D0) + 2D0*p*phi*wx ) + 2D0*( p**3*(phi**3/2D0+1D0) + 3D0*p**2*phi**2*wx ) );
                endif
            endif
        endif
        
        do i = 0, N-5
            c(i+4) = -(R**8 * p**3 * phi**2 * wy) / ((i+2) * (i+3) * (i+4)**2 * (i+5)) * c(i) + ( R**6*p**2*phi*( p*phi*(i+2.5D0)+2*wy*(phi/2+1) ) )/((i+3) * (i+4)**2 * (i+5)) * c(i+1) - ( R**4*p*( p*phi*(phi/2D0+1D0)*(2*i+5) + phi*(2D0*wy+1.5D0*p) + wx + wy) )/((i+4)**2 * (i+5)) * c(i+2) + (R**2*( p*(2*phi+1)*(i+3) + p*(phi/2+1) + wx + wy ))/((i+4) * (i+5))*c(i+3)
        enddo
    ...
    function SerraAlg2 (x, y, sigmax, sigmay, R, delta) result (Pc)
        real(kind=8), intent(in) :: R, delta
        real(kind=8) :: x, y, sigmax, sigmay
        real(kind=8) :: p, phi, wx, wy, alpha0, upper, lower
...
! if this is not true, we swap the axes using p as dummy variable
        ...
        upper = alpha0 * ( exp( p*R**2*( phi/2D0 + (wx+wy)/p ) ) - exp(-p*R**2) ) / ( p*(1 + phi/2) + wx + wy ) 
        if (upper - lower .le. delta) then
            Pc = (/ (upper+lower)/2D0  ,  lower  ,  upper /)
        else
            N1 = 2D0 * ceiling( exp(1D0) * p * R**2 * ( 1D0 + phi/2D0 + (wx+wy)/p ) )
            N2 = ceiling( log( ( alpha0 * exp( p*R**2*(phi/2D0 + (wx+wy)/p) ) ) / ( log(2D0) * delta * p * sqrt(2D0*PI*N1) * ( 1D0 + phi/2D0 + (wx+wy)/p ) ) ) )
            n = max(N1, N2) - 1
            Pc(1) = Serra (x, y, sigmax, sigmay, R, n)
            upper = alpha0 * exp( -p*R**2) * (p*R**2)**(n+1) / ( p * factorial(n+1) )
            lower = alpha0 * exp( p*R**2*(phi/2D0 + (wx+wy)/p) ) * ( p*R**2 * ( 1 + phi/2D0 + (wx+wy)/p ) )**(n+1) / ( p*( 1 + phi/2D0 + (wx+wy)/p ) * factorial(n+1)  )
